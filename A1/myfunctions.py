...
def show_graph():
    ...
    form_intersections()
    # print(list_st_name)
    # print(f"The vertex list is: {vertex_list}")
    # print(f"The intersection list is: {inter_list}")
    cleanup_inter_lines()
    # print(f"The new inter lines list: {intersection_lines2}")

    form_edge_list()

    # print(f"The edge list is: {edge_list}")
    # print(f"inter street names list: {inter_street_dict}")

    print("V = {")
    for i in range(len(vertex_list)):
        print(f"{i+1} : {vertex_list[i]}")
    print("}")
            ...
            for street2 in street_list:
                if street2 != st_name and list_st_name.index(street2) > list_st_name.index(st_name):
                    ...
                    for j in range(0, len(street_list[street2]) - 1):
                        ...
                        # print(f"Line1: {p1.x}, {p1.y} --> {p2.x}, {p2.y} ")
                        # print(f"Line2: {p3.x}, {p3.y} --> {p4.x}, {p4.y}")
...
def intersection_pt(x1, y1, x2, y2, x3, y3, x4, y4):
    if not is_parallel(x1, y1, x2, y2, x3, y3, x4, y4):
        ...
        if determinant != 0:
            ...
            p1 = Point(x1, y1)
            p2 = Point(x2, y2)
            if on_segment(p1, p2, midpt):
                update_intersection_list(int(inter_x), int(inter_y))
                ...
                # print(f"The names of streets for an intersection pt: {street1}, {street2}")

                # inter_street_dict[(inter_x, inter_y)] = [street1, street2]
                ...
                # print(f"Intersection-street dict is: {inter_street_dict}")

                # Update all direct line segments from intersection point
                not_line_list.append([(x1, y1), (x2, y2)])
                not_line_list.append([(x3, y3), (x4, y4)])

                intersection_lines.append([(x1, y1), (int(inter_x), int(inter_y))])
                intersection_lines.append([(int(inter_x), int(inter_y)), (x2, y2)])
                intersection_lines.append([(x3, y3), (int(inter_x), int(inter_y))])
                intersection_lines.append([(int(inter_x), int(inter_y)), (x4, y4)])

                # print(f"The Not LL is: {not_line_list}")
                # print(f"The Inter LL is: {intersection_lines}")
            else:
                pass
...
def cleanup_inter_lines():
    # print(f"The number of intersection pts is: {len(inter_list)}")
